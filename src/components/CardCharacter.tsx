        ...
        results.map((item, index) => {
          ...
          return (
            <Link
              ...
              key={item.id}
              className="max-w-[300px] mx-auto flex justify-center bg-gradient-to-t from-slate-800 via-slate-600 to-transparent rounded-b-2xl transition-all duration-300 hover:scale-105"
            >
                  ...
                  <div className="flex flex-col w-full">
                    <div className="flex justify-between my-2">
                      ...
                      <span className="flex justify-end self-center">
                        {handleStatus(item.status) === "vivo" && (
                          <BsCircleFill className="fill-green-500" />
                        )}
                        {handleStatus(item.status) === "morto" && (
                          <BsCircleFill className="fill-red-500" />
                        )}
                        {handleStatus(item.status) === "desconhecido" && (
                          <BsCircleFill className="fill-yellow-500" />
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between flex-wrap gap-1 text-slate-100">
                      <h2 className="w-full self-center font-bold text-base sm:text-lg">
                        {item.name}
                      </h2>
                      <span className="text-sm capitalize">
                        Espécie: {handleSpecies(item.species)}
                      </span>
                      <span className="text-sm capitalize">
                        Gênero: {handleGender(item.gender)}
                      </span>
                    </div>
                  </div>
                </div>
