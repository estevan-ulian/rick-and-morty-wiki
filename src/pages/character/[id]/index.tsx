      ...
      <Section>
        <Container>
          <div className="w-full flex flex-col sm:flex-row gap-4 justify-center items-center mt-4">        
            ...
            {loading ? <AiOutlineLoading3Quarters size={40} className="animate-spin w-[300px]" /> :
              <Image src={img} width={300} height={300} alt={character?.name} layout='fixed' className={`rounded-lg`} />
            }
            </div>
            <div className="w-full sm:w-auto flex flex-col items-center sm:items-start gap-2">
              <div className="flex items-center gap-1">
                <span className="capitalize flex items-center gap-2">
                {handleStatus(characterStatus) === 'vivo' && <BsCircleFill className="fill-green-500" />}
                {handleStatus(characterStatus) === 'morto' && <BsCircleFill className="fill-red-500" />}
                {handleStatus(characterStatus) === 'desconhecido' && <BsCircleFill className="fill-yellow-500" />} 
              ...
              <div className="flex flex-col items-center sm:items-start gap-4 mt-2">
                ...
                <span className={`text-lg text-center sm:text-left ${loading ? 'flex items-center gap-2' : ''}`}>
                <strong>Primeira aparição em:</strong>
                {
                  loading ? <AiOutlineLoading3Quarters size={16} className="animate-spin" /> :
      ...
      <Section>
        <Container>
          <div className="w-full flex flex-col gap-1">
            ...
            <span className="text-center my-2 text-slate-800"><strong>{character.name}</strong> aparece em {character.episode?.length} {character.episode?.length > 1 ? 'episódios' : 'episódio'}.</span>
            <div className="flex flex-wrap gap-2 justify-center mt-4 w-full">
              {loading ? <AiOutlineLoading3Quarters size={40} className="animate-spin" /> : episodes?.map(episode => {
                return (
                  <Link href={`/episode/${episode.id}`} key={episode.id}>
                    <a className="flex justify-center items-center text-center rounded-md border border-slate-300 px-2 py-1 transition-all duration-300 hover:bg-slate-500 hover:border-slate-500 hover:text-white">
